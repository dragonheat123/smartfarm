...
with open('C:/Users/Brian/Desktop/25102017/try.csv', "w", newline='') as csv_file:
...
pin = 2                                 ## Pin for DHT Sensor   
sensor = Adafruit_DHT.DHT22

#######define working functions
def setbluelighting(freq,d):
    GPIO.setup(20,GPIO.OUT)  #for Blue
    lightingb = GPIO.PWM(20,freq)
    lightingb.start(d) #Duty Cycle

def setredlighting(freq,d):
    GPIO.setup(21,GPIO.OUT)  #for Red
    lightingr = GPIO.PWM(21,freq)
    lightingr.start(d) #Duty Cycle
...
temp, humd = 0,0
on_time = datetime.datetime(2017,11,3,17,25).timestamp()
off_time = 0
oncycle = 12
period = 24
cycle_state = 0
...
#        getweight()   #returns val1 and val2 for 2 load cell
    ...
    now = datetime.datetime.now()
    if now.hour==14 and now.minute==25 and watering_state == 0:
        print('go time')
        watering_state = 1
    ...
    if cycle_state == 1 and time.time()<off_time and time.time()>on_time:  ###on stage
        print('set pwm high')
        setredlighting(1000,66)
        setbluelighting(1000,66)
    ...
    if cycle_state == 0 and time.time()<on_time and time.time()>off_time:
        print('set pwm low')
        setredlighting(1000,100)
        setbluelighting(1000,100)      
